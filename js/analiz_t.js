...
let shName = 'FormAnswer1';

// let rivenCriterii = [53, 93, 133, 173]
...
let readAnalizT = (s2)=>{
    gl_data = []
    data = ''
    console.log(gl_data.length)
    $.ajax({
        dataType: "json",
        url: url + 'getblock/'+key+'/'+shName,
        data: data,
        async: true, 
        ...
        btn.addEventListener('mouseup', function() {
            ...
            dd_keys.forEach(key => {                 
                ...
                if (vopros !== undefined && vopros.slice(0,1)>='1' && vopros.slice(0,1)<='9'){
                    ...
                    bal += b;
                    let nam = 'c'+copyToSpaсeAndRepl(vopros)+'_'+b;
                    // console.log('nam ', nam)
                    context[nam] = '✓'
                }
            });
            ...
            if (bal<maxBall*0.44) 
                riven = 'початковий';
            else if (bal<maxBall*0.63) 
                riven = 'середній';
            else if (bal<maxBall*0.82) 
                riven = 'достатній';
            else riven = 'високий';
...
function copyToSpaсeAndRepl(str){
    let to = str.search(' '); 
